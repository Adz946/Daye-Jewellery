...
import TypeSelector from './filter/TypeSelector';
...
export default function FilterBar({ classes, filters, updaters }) {
    ...
    return (
        <div className={`${classes} flex flex-col rounded-lg shadow-inner-custom`}>
            {/* SORT */}
            ...
            <div className='p-2 gap-6 flex flex-col'>
                ...
                <div className='p-2 gap-4 flex flex-row text-center justify-between'>
                    <input type='number' placeholder='Min' value={filters.price?.min || ""} className='w-2/5 border-b-2 border-light'
                        onChange={(e) => updaters.updatePrice(parseInt(e.target.value) || 0, filters.price?.max || 10000)} />
                    ...
                    <input type='number' placeholder='Max' value={filters.price?.max || ""} className='w-2/5 border-b-2 border-light'
                        onChange={(e) => updaters.updatePrice(filters.price?.min || 0, parseInt(e.target.value) || 10000)} />
                </div>
            </div>
